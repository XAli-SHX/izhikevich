...
from neurons import Neuron, IzhikevichNeuron
...
# Modify the create_neurons function
def create_neurons(structure, weights, biases, steps, dt, net_type, neuron_type='regular_spiking'):
    neuron_array = []
    neuron_number = 0
    ...
    for layer in range(1, len(structure)):
        for neuron in range(structure[layer]):
            if net_type == 'SNN':
                neuron_array.append(IzhikevichNeuron(layer, weights[neuron_number], biases[neuron_number],
                                                     steps, dt, neuron_type))
            else:
                neuron_array.append(Neuron(layer, weights[neuron_number], biases[neuron_number]))
            neuron_number += 1
